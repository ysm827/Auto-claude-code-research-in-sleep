...
- **2026-05-04** — ![FIX](https://img.shields.io/badge/FIX-orange?style=flat-square) 🪲 **`/research-wiki` and 8 caller skills now resolve helper via fallback chain** ([#204](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/204)). Fixes a real community bug where running W1 (`/research-pipeline` → `/idea-discovery` → `/idea-creator`) for a week left the user's `research-wiki/` empty (only the bare init state). Root cause: 32 invocations across 9 CC-side SKILLs (`/research-wiki`, `/idea-creator`, `/result-to-claim`, `/research-lit`, `/arxiv`, `/alphaxiv`, `/deepxiv`, `/exa-search`, `/semantic-scholar`) hard-coded `python3 tools/research_wiki.py …`. After `bash tools/install_aris.sh` the project has `<project>/.aris/tools/research_wiki.py` (symlink, [#192](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/192)) but NO `<project>/tools/`, so the hard-coded path silently exited 2 ("file not found"); the caller proceeded without the wiki side-effect, the user saw no error. Same root cause as [#193](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/193) fixed for `experiment-queue` — we said we'd fix the rest of the helpers and didn't. **Fix**: each SKILL now uses a 3-layer resolution chain (`.aris/tools/` → `tools/` → `$ARIS_REPO/tools/`) codified in [`shared-references/wiki-helper-resolution.md`](skills/shared-references/wiki-helper-resolution.md). `/research-wiki` hard-fails on missing helper; the 8 caller skills warn-and-skip the wiki side-effect (primary output of idea ranking / claim verdict / paper summary still produced). `/research-wiki init` now delegates to the helper so `query_pack.md` (which the prose init was missing) has one source of truth. Codex `gpt-5.5 + xhigh` review caught 4 additional sites I missed (canonical-doc bible, helper-side backfill hint, Codex-mirror incomplete fallback, Python 3.9 ImportError on `Path | None`). **Manual-copy workaround preserved**: layer 2 of the chain is exactly `<project>/tools/research_wiki.py`, so users who manually `cp`-installed the helper as a temporary fix continue to work post-merge — verified by `tests/test_research_wiki_helper_resolution.py::test_layer2_manual_copy_from_subdir`. **⚠️ Existing ARIS users**: re-run `bash tools/install_aris.sh` (or re-`cp` the helper if you used the manual workaround) to pick up `tools/research_wiki.py`'s `from __future__ import annotations` fix — without it the helper crashed on Python 3.9 (macOS system default), independent of the resolution-chain bug.
