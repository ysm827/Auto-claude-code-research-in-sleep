...
- **2026-05-02** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🧿 **Oracle MCP upstream PR queue is the first place to check** ([reviewer-routing.md](skills/shared-references/reviewer-routing.md)). When you invoke `— reviewer: oracle-pro` (especially `o3-deep-research` / `gpt-5.5-pro` paths), check [`steipete/oracle/pulls`](https://github.com/steipete/oracle/pulls) for in-flight fixes that may affect your run — model routing, browser-mode auth, rate-limit handling, new model aliases, etc. ARIS does not vendor Oracle MCP; you run the published `@steipete/oracle` from npm. If behavior surprises you, the upstream PR queue is the first triage stop before opening an issue here
- **2026-05-02** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 💎 **`/gemini-search` default bumped to `gemini-3-pro-preview`** ([SKILL.md](skills/gemini-search/SKILL.md)). The strongest Gemini option is now the out-of-box default, no override needed. ⚠️ **Action required**: requires `gemini-cli` v0.40+ (run `gemini --version`; upgrade with `npm i -g @google/gemini-cli` if older). If you must stay on the legacy CLI, override with `/gemini-search "topic" — model: gemini-2.5-pro`. Other available overrides: `— model: gemini-3-flash-preview` (faster / higher quota / less capable than Pro), `— model: auto-gemini-3` (auto-routes within Gemini 3 family by load). Both `mcp__gemini-cli__ask-gemini` and the CLI fallback accept all aliases verbatim
