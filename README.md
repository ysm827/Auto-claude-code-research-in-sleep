...
- **2026-05-01** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🧾 **`/citation-audit` opt-in `--uncited` flag** ([#187](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/187), [SKILL.md](skills/citation-audit/SKILL.md)). Surfaces bib entries with no `\cite{}` referencing them — detect-only, never sent to reviewer (no extra web-lookup cost), never alters `verdict` / `reason_code` / `summary`. **Zero behavior change** when flag is unset (`details.uncited_entries` field omitted). Suggestion default `prune`; only `check` with concrete local evidence (TODO comment, recent `git diff`). Built via delegated-agent + maintainer-fixup workflow (both commits visible in PR history)
