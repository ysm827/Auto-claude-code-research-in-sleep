...
- **2026-05-01** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🧾 **`/citation-audit` 新增 opt-in `--uncited` flag** ([#187](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/187), [SKILL.md](skills/citation-audit/SKILL.md))。扫出 bib 里没人 `\cite{}` 的死条目——纯检测、不送 reviewer（无 web-lookup 开销），不改 top-level verdict。**不传 flag 时零行为变化**（`details.uncited_entries` 字段不存在）。建议默认 `prune`，只在有本地具体证据（TODO 注释、最近 `git diff` 里删了 `\cite`）才标 `check`。delegated-agent + maintainer-fixup 双 commit 工作流（PR 历史里可见两个 commit）
