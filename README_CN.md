...
- **2026-05-04** — ![FIX](https://img.shields.io/badge/FIX-orange?style=flat-square) 🪲 **`/research-wiki` 和 8 个调用方 skill 改用 fallback chain 解析 helper** ([#204](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/204))。修复一个真实社区 bug：用户跑 W1 (`/research-pipeline` → `/idea-discovery` → `/idea-creator`) 一整周，`research-wiki/` 一直停在 init 后的空状态。根因：9 个 CC-side SKILL（`/research-wiki`、`/idea-creator`、`/result-to-claim`、`/research-lit`、`/arxiv`、`/alphaxiv`、`/deepxiv`、`/exa-search`、`/semantic-scholar`）共 32 处写死 `python3 tools/research_wiki.py …`。跑过 `bash tools/install_aris.sh` 之后，项目里有 `<project>/.aris/tools/research_wiki.py` symlink（[#192](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/192)）但**没有** `<project>/tools/`，写死的路径直接 silently exit 2（"file not found"），调用方继续往下走、但 wiki 副作用从未发生、用户看不到任何错误。和 [#193](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/193) 修 `experiment-queue` 是同一个根因——当时说要把剩下的 helper 都修，没修干净。**修复**：每个 SKILL 改用 3 层 resolution chain（`.aris/tools/` → `tools/` → `$ARIS_REPO/tools/`），canonical pattern 写在 [`shared-references/wiki-helper-resolution.md`](skills/shared-references/wiki-helper-resolution.md)。`/research-wiki` hard-fail；8 个调用方 skill 走 warn-and-skip——主输出（idea 列表 / claim 判决 / paper summary）照常给用户，只是 wiki 副作用单独 skip 并打 warning。`/research-wiki init` 现在 delegate 到 helper（之前 prose init 漏了 `query_pack.md`）。Codex `gpt-5.5 + xhigh` review 还抓到 4 个我漏的点（canonical 文档圣经、helper 端 backfill hint、Codex-mirror 不完整 fallback、Python 3.9 上 `Path | None` ImportError）。**手动 cp 临时方案被保留**：chain 第二层就是 `<project>/tools/research_wiki.py`，已经手动 `cp` 过 helper 的用户合 PR 后**继续 work**——`tests/test_research_wiki_helper_resolution.py::test_layer2_manual_copy_from_subdir` 验证。**⚠️ 已装 ARIS 用户**：重跑 `bash tools/install_aris.sh`（或如果走的是手动 cp 路线，重新 `cp` 一次 helper）以拿到 `tools/research_wiki.py` 的 `from __future__ import annotations` 修复——不修这个的话 helper 在 Python 3.9（macOS 系统默认）上 ImportError，和 resolution-chain bug 是独立的两个 bug。
