...
- **2026-05-02** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🧿 **Oracle MCP 上游 PR 队列是第一排查点** ([reviewer-routing.md](skills/shared-references/reviewer-routing.md))。用 `— reviewer: oracle-pro`（尤其是 `o3-deep-research` / `gpt-5.5-pro` 路径）时，先看 [`steipete/oracle/pulls`](https://github.com/steipete/oracle/pulls) 有没有 in-flight fix 影响你的 run——model routing、browser-mode auth、rate-limit 处理、新 model alias 等。ARIS 不 vendor Oracle MCP，你跑的是 `@steipete/oracle` npm 发布版。行为意外时，**上游 PR 队列是开 issue 之前的第一排查点**
- **2026-05-02** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 💎 **`/gemini-search` 默认升级到 `gemini-3-pro-preview`** ([SKILL.md](skills/gemini-search/SKILL.md))。最强 Gemini 现在是开箱默认，不需要手动 override。⚠️ **需要操作**：需要 `gemini-cli` v0.40+（跑 `gemini --version` 查；老版本用 `npm i -g @google/gemini-cli` 升级）。如果必须留在旧 CLI，用 `/gemini-search "topic" — model: gemini-2.5-pro` override。其他可用 override：`— model: gemini-3-flash-preview`（更快 / 配额更高 / 能力比 Pro 弱）、`— model: auto-gemini-3`（在 Gemini 3 家族内按负载 auto-route）。`mcp__gemini-cli__ask-gemini` 和 CLI fallback 都直接接受所有别名
